...
//Get the topics on this page and Sticky statut
if ($count > 0)
{
	//calculate the pages
	$pages = $pagies->calculate_pages($count, $perPage, $p);

	$topics_res = $DB->prepare("SELECT * FROM `wcf_topics` WHERE `forum` = :forum ORDER BY `lastpost_time`, `stick` DESC LIMIT ".$pages['limit'].";");
